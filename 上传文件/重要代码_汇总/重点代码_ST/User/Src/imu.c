	...
	while(1){
		...
		if((fabs(k_roll)>1.0||fabs(k_pitch)>1.0)){
			if(k_roll>1.0)
				...
				step_roll=k_roll*one_point;

			if(k_pitch>1.0)
				step_pitch=one_point;
			else if(k_pitch<-1.0)
				step_pitch=-one_point;
			else
				step_pitch=k_pitch*one_point;
			to_state(hexa_p,(*hexa_p).roll+step_roll,(*hexa_p).pitch+step_pitch,0.0,(*hexa_p).tz);

...
void stand(struct hexapod* hexa_p ,int N)
{
	float coord[6][3];
	for(int i=1;i<=N;i++){
		change_z_step(coord,hexa_p,0.0,-(*hexa_p).height,i, N);
...
void crouch(struct hexapod* hexa_p ,int N)
{
	float coord[6][3];
	for(int i=1;i<=N;i++){
		change_z_step(coord,hexa_p,-(*hexa_p).height,0.0,i, N);
		sports_6lg(coord,hexa_p);
		HAL_Delay(30);
	}
	(*hexa_p).tz=0.0;
...
void wheel2six(struct hexapod* hexa_p)
{
	...
	//放腿
	for(int i=1;i<=N;i++){
		change_z_step(coord,hexa_p,0.0,-250.0,i, N);
		sports_6lg(coord,hexa_p);
		HAL_Delay(30);
	}
	//收起轮足，由于舵机是270°舵机，因此转动角度有所区别
	for(int i=1;i<=N;i++){
		SERVO_SPT(9,bias[9]+(-60.0+120.0*((float)i)/N));//增大
		SERVO_SPT(10,bias[10]+(60.0-120.0*((float)i)/N));//减小
		SERVO_SPT(21,bias[21]+(60.0-120.0*((float)i)/N));//减小
		SERVO_SPT(22,bias[22]+(-60.0+120.0*((float)i)/N));//增大
		HAL_Delay(10);
	}
	//抬腿
	for(int i=1;i<=N;i++){
		change_z_step(coord,hexa_p,-250.0,-(*hexa_p).height,i, N);
		sports_6lg(coord,hexa_p);
		HAL_Delay(20);
	}
	(*hexa_p).tz=-(*hexa_p).height;
...
void six2wheel(struct hexapod* hexa_p)
{
	...
	//放腿
	for(int i=1;i<=N;i++){
		change_z_step(coord,hexa_p,-(*hexa_p).height,-250.0,i, N);
		sports_6lg(coord,hexa_p);
		HAL_Delay(20);
	}
	//放下轮足，由于舵机是270°舵机，因此转动角度有所区别
	for(int i=1;i<=N;i++){
		SERVO_SPT(9,bias[9]+(60.0-120.0*((float)i)/N));
		SERVO_SPT(10,bias[10]+(-60.0+120.0*((float)i)/N));
		SERVO_SPT(21,bias[21]+(-60.0+120.0*((float)i)/N));
		SERVO_SPT(22,bias[22]+(60.0-120.0*((float)i)/N));
		HAL_Delay(10);
	}
	//抬腿
	for(int i=1;i<=N;i++){
		change_z_step(coord,hexa_p,-250.0,0.0,i, N);
		sports_6lg(coord,hexa_p);
		HAL_Delay(30);
	}
//	(*hexa).tz=0.0;
...
void to_state(struct hexapod* hexa_p,float roll,float pitch,float yaw,float tz)
{
	...
	for(int i=1;i<=N;i++){
		...
		temp_yaw+=step_yaw;
		temp_tz+=step_tz;
		if((fabs(temp_roll)>M_PI/18)||(fabs(temp_pitch)>M_PI/18)||(fabs(temp_yaw)>M_PI/18)){
			break;
		}
...
void straight_4lg(struct hexapod* hexa_p)
{
	...
	while(1){
		...
		for(int i=0;i<4;i++){
			for(int j=1;j<=N;j++){
				straight_4lg_leg_step(coord,hexa_p,theta,i,j,N);
		...
		/*移动整个身体*/
		for(int j=1;j<=N;j++){
			straight_4lg_body_step(coord,hexa_p,theta,j,N);
			sports_4lg(coord,hexa_p);
			HAL_Delay(30);
		}
	}
